# 148 · SSAIR optimizer tier

**Avenue:** Risk (the "faster than Swift/Go" perf substrate) · **Type/Lifecycle:** `perf · mostly-shipped`
· **Size:** L · **Status:** tier **built** (M7, 2026-08-24); the `148.x` sub-items below are the open
tail plus the shipped/terminal record · **Source:** dissolved from `ssair-backlog.md` (2026-08-25)

The mid-level optimizer tier: **SSAIR** (a lower control-flow-explicit SSA IR) plus the
language-aware passes that run on it. The tier itself shipped; this task holds its sub-item backlog
at `148.x` granularity — passes, analyses, IR/infra, validation. The as-built design + decisions
live in `../../internals/ssair.md` + `../../internals/memory-model.md` §6.1; cross-cutting
language/feature deferrals live in the [task index](../tasks.md). The `[§7.x]` tags are the
(retired) `m7-spec` milestone-phase labels, kept as historical provenance; `[new]` items were first
raised here.
...
- **Scalar promotion — trivial-param elimination** `[S · deferred]` `[§7.3.1 A3]` — a loop-invariant
  ...
  shared-Braun-engine extraction (§148.6).
...
## 148.2 · Devirtualization (§7.4)

- **Cross-function devirt** `[M · deferred]` `[§7.4]` — an `any I` arriving as a function *parameter* has
  no locally-visible conformer, so it stays `witness`. Largely subsumed once the caller inlines (§148.3);
  otherwise an interprocedural conformer analysis.

## 148.3 · Inlining & specialization (§7.5)
...
## 148.4 · Bounds-check elimination (§7.5 residual)
...
- **Dominating-redundant + constant-fold BCE** `[M · deferred]` `[§7.5]` — the retained residual:
  ...
  statically known. Needs a dominator tree (§148.6) + light value-numbering, no loop reasoning. The
  loop-bound case is descoped in favor of `for … in` bounds-check-free-by-construction ([117](117-for-in-iteration.md)).

## 148.5 · Classic SSA optimizations (only where language-aware)

Screen each against the guiding principle above — most plain scalar opts are LLVM's job.

- **Self-tail-call → loop** `[S · evaluate]` `[TCO]` — rewrite a function's tail call to
  ...
  [129 tail-call optimization](129-tail-call-optimization.md).)
...
- **LICM (language-aware)** `[M · evaluate]` — LLVM does scalar LICM; the tier-only case is hoisting a
  loop-invariant *allocation* or barrier (overlaps §148.1). Evaluate against LLVM's residual.

## 148.6 · Analysis framework & infrastructure
...
- **Shared SSA-construction (Braun) engine** `[S–M · deferred]` `[§7.3.1]` — `ScalarPromotion`
  ...
  trivial-param cleanup (§148.1) reuse.

## 148.7 · Verifier & correctness (§7.6.2)
...
## 148.8 · IR format, tooling, observability

- **IR hardening — versioned round-trippable SSAIR format + per-stage I/O** `[M–L · decide-early]`
  `[142]` — a durable text format + a `--start-from` counterpart to `--stop`, enabling per-stage
  unit tests and stage-as-invariant (golden IR in → expected IR out). Cheapest to set while SSAIR is
  young; flagged decide-early for M7. Cross-cut: [142 IR + pipeline hardening](142-ir-pipeline-hardening.md).
- **Per-pass firing counts** `[S · deferred]` `[§7.6.1]` — each pass reports how many sites
...
## 148.9 · Perf validation & benchmarks
...
- **Randomized differential (T7)** `[L · M12]` `[§7.0.5 T7]` — a generated-program fuzzer, tier-on vs off
  ...
  [105 concurrency hardening](105-concurrency-hardening.md).

## 148.10 · Terminal — retire the NOIR oracle (§7.7)
