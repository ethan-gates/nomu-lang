...
- **Risk** — validate the risky bets Nomu rests on. Headline: the **LXR** RC-hybrid collector
  (footprint endgame). The concurrency-model completion, self-hosting runtime, and fiber-stack
  strategy feed the same "prove the hard thesis" goal.
...
matters, lives in the roadmap horizon, not here. Drill-down inside a task uses the number —
...
| 125 | [Unsafe raw memory / raw pointers](tasks/125-unsafe-raw-memory.md) | Infra | L | needs-design (► decide-early, before stdlib) |
...
| # | Task | Avenue | Size | Status |
| --- | --- | --- | --- | --- |
| 127 | [LXR collector (footprint endgame)](tasks/127-lxr-collector.md) | Risk | XL | evaluate (gated on benchmark-scale stdlib) |
| 128 | [Self-hosting the runtime](tasks/128-self-hosting-runtime.md) | Risk | XL | needs-design (design-early, build-late) |
...
Plus the design-early notes on [self-hosting](tasks/128-self-hosting-runtime.md) (runtime-subset mechanism +
raw pointers) and [tail-call optimization](tasks/129-tail-call-optimization.md) (guarantee-vs-best-effort).
