...
- **Actors** are owning GC handles with reference-driven lifetime (collected when unreferenced-and-idle). You interact by **fire-and-forget message-send** written as a method call (no suspension marker); handlers don't return to the sender. Getting a value back is an explicit request-reply — an `ask` built on the continuation (§3) that still reads as a plain blocking call, not a handler return (§9). Exact actor-access syntax open (§9). No `weak`, no dangling.
...
- **Actors sit outside the cancellation tree.** Actors are independent entities with reference-driven lifetime, not nodes in the structured scope tree. Cancelling a fiber that *sent* a message does **not** abort the actor's in-flight handler (that would corrupt actor state). A fiber **awaiting an actor reply** is still cancellable — the wait unwinds and the eventual reply is dropped (identical to resume-after-cancel, §3). — **Decided.** Actor *shutdown* (drain-then-collect) is settled in the actor surface (§9).
...
- **Interaction = method-call syntax, fire-and-forget message-send.** `account.deposit(amount)` reads like a call; underneath it enqueues a message on the actor's mailbox and **returns immediately** — the handler runs later on the actor's turn, and the sender does not wait for it. Handlers **do not return values to the sender** (request-reply is a separate explicit pattern, below). Legibility comes from the variable's actor type, consistent with the markerless model (§1). — **Decided (fire-and-forget: revised 2026-08-12, was "blocks until the handler returns").**
- **One semantics: fire-and-forget, no blocking variant.** Every handler call is a non-blocking send; the sender never waits on the handler. There is no returning/blocking call form. The mailbox is unbounded to start (a send always succeeds); **bounded-mailbox backpressure** (a send blocks when full, Go-buffered-channel style) is a future option (§10), not the default. — **Decided (revised 2026-08-12).**
- **Request-reply is an explicit pattern, not a handler return.** A handler never returns a value to its sender. To get an answer, the sender passes a **continuation** (§3) — or a reply address — in the message, and the handler resumes it with the answer. A stdlib `ask` helper wraps that into one colorless blocking call for the asker (the continuation's stated job: manufacture a colorless blocking call from a callback shape, §3), so a value read still *reads* like an ordinary blocking call — but the request-reply is **built from the continuation, explicit, and never a handler's return type**. Precedent: Erlang (`!` primitive, `gen_server:call` a library over it), Akka (`tell` primitive, `ask` over Futures), Go (channel send is a void statement, reply is a separate channel). — **Decided 2026-08-12.**
...
- **Non-reentrant.** A handler runs to completion before the next message, so state is touched serially and invariants hold across the whole handler. A handler that blocks inline (an `ask`, I/O) serializes the actor deliberately. — **Decided.**
...
- **Arguments are shareable** — they cross the actor boundary, so the shareability rule requires it (§1, §5). No results cross back (handlers don't return to the sender); an `ask` reply is itself a message, so it is shareable by the same rule. — **Decided (follows from the shareability rule).**
...
- **Message-send, not a mutex.** An actor owns a **mailbox** — an MT-safe FIFO of messages. A call enqueues a message and returns (fire-and-forget); the actor's drain runs the handler later — the sender is never parked on the handler. This is what makes fire-and-forget, offload, and non-reentrant run-to-completion expressible. A mutex gives none of them: it runs the handler on the caller's own fiber and deadlocks on re-entry. The M3.4 `{ header, fields…, mu }` mutex actor was a belief-milestone scaffold — now superseded. (Request-reply, when used, parks the *asker* on its own continuation, §3 — a separate mechanism, not part of the send.)
- **Pooled, on-demand handler fibers — no dedicated per-actor fiber.** An idle actor is just its state + mailbox; it holds no fiber and no OS stack. When a message arrives at an idle actor, its mailbox is marked *scheduled* and a **drain task** is submitted to the scheduler; a fiber **rented from a single program-global pool** picks it up, runs handlers to completion until the mailbox empties, then clears *scheduled* and **returns itself to the pool**. The pool is a retained free-list of reusable worker fibers (not spawn-per-drain): a worker loops — drain an actor, park back on the free-list, resume on the next rental with a freshly-assigned actor — so fiber stacks are warm-reused, never owned by any actor. Over its life an actor may be drained by different pool fibers on different rentals. Serialization and non-reentrancy come from the **single-drain invariant** (at most one scheduled drain per actor at a time), not a lock; the pool parallelizes *across* actors, never within one. The pool is elastic (grows to a cap when every worker is blocked mid-handler). Actors don't each own a thread-sized stack; the pool is sized to simultaneously-*busy* actors, not to how many exist. (Erlang/Akka dispatcher precedent.)
- **Ordering: per-sender FIFO.** A single sender's messages to an actor are handled in send order (they enter the one FIFO mailbox in the sender's program order). Cross-sender order — two fibers sending concurrently — is arbitrary, as in every actor system (concurrent sends have no defined order). Ordering and serialization come from the single FIFO mailbox + the single-drain invariant, not from which fiber does the draining. The drain's empty-check races a concurrent send: on seeing the mailbox empty a worker clears *scheduled*, then **re-checks** before returning to the pool, re-acquiring the drain if a message slipped in — preserving both order and single-drain.
- **An inline-blocking handler holds its borrowed fiber parked** — on whatever it awaits (an `ask` reply, a timer, I/O); its roots are scanned via the live-fiber registry (6.2.2), like any parked fiber. The pool is elastic; the actor is simply busy until the handler completes.
- **Park protocol: lock-handoff (M6 · 6.4, built).** A parking-heavy concurrency workload surfaced a latent M:N race: a fiber that unlocked the scheduler lock before `swapcontext` finished saving its context could be re-queued by a waker mid-save → SIGBUS / lost-wakeup deadlock. Fixed by holding the single scheduler lock (`rt_queue_mu`) across every fiber↔scheduler switch — the scheduler switches in with the lock held, a fiber releases it only after switch-in and re-acquires it to park, so a fiber becomes wakeable only once its context is fully saved. Applied uniformly to spawn/join, the timer sleep, the I/O poller, and pool-worker rental — hardening the pre-existing park sites too.
...
**Residual (§10):** all spellings (the actor keyword, handler/message declaration, the `stop` form); the **`ask` request-reply helper surface** (a colorless blocking call over the continuation, §3); whether the mailbox is unbounded or bounded (backpressure); and the deadlock policy for a cyclic `ask` (an actor asking, transitively, back into itself — detect / timeout). Some rides on generics and the error model.
...
- **Actor surface** — **Decided (§9), fire-and-forget revision 2026-08-12:** method-call syntax over **fire-and-forget** message-send; handlers never return to the sender (request-reply is an explicit `ask` over the continuation §3, not a handler return — no implicit blocking); non-reentrant; slow handlers spawn to stay responsive; actor is a scope for spawned work; pending messages root the actor → drain-then-collect. **Implementation model (§9):** async mailbox + pooled on-demand handler fibers (no dedicated per-actor fiber, no mutex); the M3.4 mutex actor is a superseded scaffold; teardown falls out of the moving GC (no per-actor weak handle). Residual: spellings, the `ask` helper surface, mailbox bound, cyclic-`ask` deadlock policy.
