...
- **Actors** are owning GC handles with reference-driven lifetime (collected when unreferenced-and-idle); reads look like ordinary access/calls (no suspension marker), exact actor-access syntax open (§9). No `weak`, no dangling.
...
- **Interaction = method-call syntax, message-send underneath.** `account.deposit(amount)` / `let b = account.balance()` reads like a call; underneath it enqueues a message and, for a returning call, waits for the reply. Legibility comes from the variable's actor type, consistent with the markerless model (§1). — **Decided.**
- **One semantics: a call blocks until the handler returns.** Uniform for void and returning calls; the wait doubles as **backpressure** — a slow actor throttles its senders, bounding the mailbox. — **Decided.**
- **Workers offload; the author picks per handler.** A handler facing slow work chooses to compute inline (the actor serializes on it — the choice when serialization is the goal) or to **spawn** the work and return an ack (the actor stays free; the result returns via a follow-up message). Fire-and-forget is a short handler that spawns and returns; `spawn`-ing the call itself skips even the ack. The language supplies the call + spawn primitives and leaves the policy to the actor's author. — **Decided.**
- **Non-reentrant.** A handler runs to completion before the next message, so state is touched serially and invariants hold across the whole handler. The offload idiom keeps the actor responsive under this rule; a handler that blocks inline serializes deliberately. — **Decided.**
...
- **Arguments and results are shareable** — they cross the actor boundary, so the shareability rule requires it (§1, §5). A consequence, restated for clarity. — **Decided (follows from the shareability rule).**
...
**Implementation model — async mailbox + pooled handler fibers (Decided 2026-08-12).** The runtime that backs this surface is the async message-send model above, not a shared lock. Pinned:

- **Message-send, not a mutex.** An actor owns a **mailbox** — an MT-safe FIFO of messages. A call enqueues a message; a returning call parks the sender until the handler unparks it with the reply (the suspension core, §2 — `mailbox.recvQ` / `park` / `unpark`). This is what makes offload, fire-and-forget, and non-reentrant run-to-completion expressible. A mutex gives none of them: it runs the handler on the caller's own fiber and deadlocks on re-entry. The M3.4 `{ header, fields…, mu }` mutex actor was a belief-milestone scaffold with the same *observable* behavior for a plain blocking call only — now superseded.
...
- **An inline-blocking handler holds its borrowed fiber parked** — on whatever it awaits (a reply, a timer, I/O); its roots are scanned via the live-fiber registry (6.2.2), like any parked fiber. The pool is elastic; the actor is simply busy until the handler completes.
- **Park protocol: lock-handoff (M6 Â· 6.4, built).** Actor↔actor message-send parks fibers on each other heavily, which surfaced a latent M:N race: a fiber that unlocked the scheduler lock before `swapcontext` finished saving its context could be re-queued by a waker mid-save → SIGBUS / lost-wakeup deadlock. Fixed by holding the single scheduler lock (`rt_queue_mu`) across every fiber↔scheduler switch — the scheduler switches in with the lock held, a fiber releases it only after switch-in and re-acquires it to park, so a fiber becomes wakeable only once its context is fully saved. Applied uniformly to spawn/join, the timer sleep, the I/O poller, the actor call, and pool-worker rental — hardening the pre-existing park sites too.
...
**Residual (§10):** all spellings (the actor keyword, handler/message declaration, explicit fire-and-forget and stop forms, the follow-up-reply pattern), and the deadlock policy for cyclic *inline-blocking* handler calls (detect / timeout). Some rides on generics and the error model.
...
- **Actor surface** — **Decided (§9):** method-call syntax over message-send; one semantics (blocks until handler returns, with backpressure); workers offload via spawn; non-reentrant; actor is a scope for spawned work; pending messages root the actor → drain-then-collect. **Implementation model Decided 2026-08-12 (§9):** async mailbox + pooled on-demand handler fibers (no dedicated per-actor fiber, no mutex); the M3.4 mutex actor is a superseded scaffold; teardown falls out of the moving GC (no per-actor weak handle). Residual: spellings, cyclic inline-call deadlock policy.
