...
> **Being dissolved (2026-07-30).** This build-sequence doc is being folded back into the settled design docs and will be deleted. **Generics are now fully specified in `generics.md`** (the authoritative as-built spec) — do not treat this file's `5.2`–`5.6` records as the generics spec. Still to move before deletion: interface mechanics + computed-property accessor spelling + `some`/`any` + covariant-`Self` erasure → `interfaces.md`; the shareability checker → `concurrency.md` §5; `Result`/error handling → `types.md` §4. Once each has a home here-below's content is redundant.
...
Sequenced last in M5; relaxes 5.1.2's blanket constraint-only rule. Depends only on 5.1 (existentials + `Self`-requirements) plus 5.4 (mono enables the `<T: I>` bound lift). Case catalog + acceptance target: `examples/self-requirements.nomu`.

**Why it's worth doing (not just for symmetry):** for a covariant interface, heterogeneous erasure is *meaningful* — `[any Cloneable]` cloned element-wise can't be written with `<T: I>` (which is homogeneous). For a contravariant interface (`Combinable`) the heterogeneous case is *incoherent* (combining a `Tally` with a `Point`), so `any I` is correctly denied and generic bounds lose nothing. 5.6 makes the rule reject exactly the incoherent case, not one interface wider.

**Decisions (2026-07-30, with Ethan):**
- **(1) Conservative position rule.** Covariant = a **bare method return** (`-> Self`) or a `{ get }`-only `Self` property. Everything else — a parameter, a `{ get set }` `Self` property, or `Self` **nested inside a function type** (either side) — is non-covariant → constraint-only. The full sign-flipping variance calculus (a `(Self) -> Bool` parameter is technically covariant) is **deferred**; the conservative rule is sound and simple.
- **(2) Include the `<T: I>` bound lift.** The exit needs `<T: Cloneable>` usable, but it's rejected today (deferred in 5.2.2 as "needs a Self-witness"). Monomorphization (5.4) specializes the bound to a concrete `T` where `-> Self` is a direct call — no witness — so the guard is now removable. 5.6 lifts it and records constraint-only conformances for bound satisfaction.
- **(3) Erase to the receiver's own existential type.** `c.clone()` on `c: any B` (where `B: A` declares the requirement) yields `any B`, not `any A`, so chaining stays in the richer type.
- Concrete and `some I` paths are **unchanged** — only the `any I` path gains erasure.

### Slices
- **5.6.1 ✅ — covariant-`Self` existentials (the core).** Position analysis; refined `any I` legality; witness emission for covariant interfaces with `-> Self` slots erased to `AnyBox` + re-box thunk; Sema return-type erasure.
- **5.6.2 ✅ — the `<T: I>` bound lift.** Constraint-only conformances made bound-satisfying; bound guard dropped; verified under mono.

**5.6 (2026-07-30) — built + green.** Closes M5. Acceptance catalog: `examples/self-requirements.nomu` (its covariant blocks now live; runs `3 4 3 42 7 point`).
- **Position analysis (Sema).** Replaced the boolean `constraintOnly`/`ownMentionsSelf` with `hasNonCovariantSelf` (+ `ownHasNonCovariantSelf`, `isBareSelf`): a non-covariant occurrence is `Self` in a **parameter**, a `{ get set }` `Self` property, or `Self` **nested in a function type** (conservative rule, decision 1). Bare `-> Self` returns and `{ get }`-only `Self` properties are covariant. Propagates through refinement.
- **`any I` legality.** `resolveExistential` rejects only `hasNonCovariantSelf` interfaces (new message names the non-covariant position); covariant-only interfaces are now legal existentials.
- **Witness emission + erasure.** `buildIRInterfaces` emits a witness table for a covariant interface (skips only `hasNonCovariantSelf`); each slot resolves `Self` with `selfAs: .existential(i.name)`, so a `-> Self` slot is `AnyBox`. Codegen `emitMethodThunk` detects an `.existential` return and **re-boxes** the concrete result (`rt_alloc` + witness instance) as `any iface`; a forward declaration of every witness instance is emitted before the thunks (the re-box is a back-reference). Sema's `any I` method-call resolves the return with `selfAs: recv.type`, so `c.clone()` on `any B` yields `any B` (decision 3).
- **Bound lift (decision 2).** `validateBounds` no longer rejects a `Self`-mentioning bound; `typeConforms` consults `allConformsTo` (all checked conformances, incl. witness-less constraint-only) so `<T: Cloneable>` / `<T: Combinable>` satisfy their bounds — discharged by mono (5.4) specializing to a concrete `T`, a direct `-> Self` call, no witness. The witness-passing path for a contravariant bound never reaches codegen (mono removes it).
- **Seams:** `Sema.swift` (position analysis; 4 gates: any-legality, bound guard, witness gating, conformance recording; `typeConforms`; call-site erasure), `CodegenIR.swift` (thunk re-box + witness-instance forward decls). Six `SemaTests` rewritten to 5.6 semantics (covariant accepted / contravariant rejected / bound accepted); `bazel test //...` green; 29 examples compile.
- **Deferred:** the full sign-flipping variance calculus (a `(Self) -> Bool` *parameter* is technically covariant) — conservative rule keeps it constraint-only; revisit if a real need appears.

**Exit — met:** `any Cloneable` is legal and `c.clone()` yields `any Cloneable` (dispatched dynamically); `any Combinable` stays rejected with the constraint-only message; both are usable as `<T: I>` / `some I`.
