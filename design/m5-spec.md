...
- Suggested slices: **5.2.1** parse + type model (`.typeParam`/`.generic`, `Box<Int>` in type position); **5.2.2** generic functions + bounds + witness-passing + arg inference; **5.2.3** generic types (`Box<T>`, `enum Option<T>`) with value witnesses; **5.2.4** prelude citizens (`Option<T>`, `Box<T>` in `stdlib.nomu`).
