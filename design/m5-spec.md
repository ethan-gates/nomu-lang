...
**Numbering.** Sections are milestone-rooted: `5.0.x` = non-implementation (scope, dependencies, decisions, risks, deferred lists) and may sit before or after the plan; `5.1`–`5.6` = the phased build plan (Phase A–F), with sub-slices like `5.1.3`. Letters (Phase A, A3) are kept as aliases in headings so older references still resolve. A `✅ built · 🔨 in progress · ⬜ not started` marker on each phase/slice shows where we are. ("M5.1"-style tags mean a *release*, never a section — the covariant-`Self` work is now just Phase F / **5.6**.)

**Build status (2026-07-26):** **5.1 (Phase A) ✅ complete** — 5.1.1/5.1.2/5.1.3 all built and green. **5.2 (Phase B) is next.**
