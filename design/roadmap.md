...
- **M4.9 — real typechecker / semantic analysis (M5 prerequisite):** stand up a genuine semantic-analysis pass — a proper internal type model (replacing the current string-typing), name/member/method resolution, expression typing, call/argument checking, conformance-ready symbol tables, and collected diagnostics — producing a typed AST that codegen consumes instead of re-deriving types in `typeOf`. Also lands **instance methods on `struct`/`enum`/`class`** (the compiler has only top-level `fun` + actor handlers today; a prerequisite for interface method requirements). Inserted because M5's interfaces/generics need real type information the codegen-embedded type tracking can't provide. Builds the `compiler.md` §1 typed mid-level IR (structured, not CFG/SSA). **Spec: `m4.9-spec.md`.**
