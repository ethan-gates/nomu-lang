...
- **M4.13 — C-source split + Nomu prelude (M5-adjacent) — DONE.** Move the emitted C runtime out of an embedded Swift string into real `.c`/`.h` files (a privileged `runtime` and the stdlib's C `core` floor for `String` + primitive ops), and add a **prelude-compilation mechanism** so a Nomu-written standard library compiles alongside user code (prepended decls under the single CU). No new syntax. The standard library is written in Nomu and recompiled each build (dogfooding + front-loads LLVM test surface); `String` stays a C primitive until the buffer primitive exists (post-M5, with collections). Packaging: embed-and-emit, so `nomuc` stays a single relocatable binary. Hard-gates M5 Phase B (the home for `Result`/`Option`); recommended before Phase A. Design + build plan: `m4.13-spec.md`.
