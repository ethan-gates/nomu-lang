...
The design forks behind these (rejected alternatives, reopen triggers) were retired with the M6 spec; the runtime-facing GC decisions (mutator granularity, safepoints, parked-fiber scanning) live in `runtime.md` §6, and the backend substrate (statepoints, seams, pass pipeline) in `compiler.md` §2. The shipped GC code (`src/gcbinding/`, `src/runtime/`) is the implementation record.
...
- **Performance realization** — escape analysis, inline value layout, monomorphization, MMTk integration (precise stack maps, barriers, object model). Compiler work, invisible to the surface (`compiler.md`).
