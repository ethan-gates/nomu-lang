...
- [internals/runtime-subset.md](internals/runtime-subset.md) — **design draft (task 149):** the mechanism that lets runtime code avoid the services it implements (no implicit alloc / barrier / safepoint, plus nosplit) — surface A (module-default subset + `nosplit fun` keyword), the internal per-function property set, the call-graph closure check + codegen guards, and the 125 allowlist. Nomu's `//go:nosplit` analog.
- [internals/selfhosted-gc.md](internals/selfhosted-gc.md) — **design draft (task 150):** bringing the GC up in Nomu one rung at a time (NoGC → mark-verify → Immix → GenImmix), each diffed against MMTk as oracle — the shared substrate reused across rungs, and **rung 1 (NoGC)** in depth: a Nomu bump allocator over 125 off-heap memory behind the existing allocation entry point, `addrspace(1)` production kept compiler-emitted, policy in Nomu under 149.
