...
- [m8-spec.md](m8-spec.md) — **working draft**: M8 implementation spec for the LLVM backend — IR production (LLVM C API), DWARF, the GC/cancellation/preemption safepoint substrate, runtime-as-library. The next build milestone (precedes M6). Forks decided in 8.0.4; phased plan 8.1–8.5 (8.1–8.4 complete). The 8.4 GC-substrate detail (statepoints, `addrspace(1)` model, stack maps, inline seams) shipped inert and its M6 carry-over lives in `m6-spec.md` §6.0.8; the dedicated 8.4 spec was retired.
