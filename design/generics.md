...
- **`<` / `>` disambiguation — Swift model.** — Decided (2026-07-25). Angle brackets are generic only in **declaration** position (`fun f<T>`, `struct Box<T>`) and **type** position (`let b: Box<Int>`). In **expression** position `<` is *always* comparison. There is **no call-site type-argument form** — `f<Int>(x)` does not exist. The parser needs no lookahead or backtracking.
- **Construction is inference-based.** You never write type arguments at a construction site; the type comes from the arguments and/or context: `Box(value: 3)` (a `Box<Int>`), `let s: Box<String> = Box(value: "hi")`. Enum construction: `Option.some(value: x)` / `.some(value: x)`, `Result.ok(value: v)` / `.err(error: e)`.
...
Inference is **bidirectional** — from argument types and from return/context — with **no explicit call-site form**; where inference can't resolve a parameter, the caller supplies **type context** (an annotation).
...
- **Associated types.** Out of the first cut, but the witness layout reserves a **type-witness slot** and the checker can model a projected type, so adding them is wiring not redesign (`interfaces.md` §4.5).
...
- **No call-site type arguments (turbofish / whitespace rule rejected).** Turbofish (`f::<Int>(x)`) is new syntax; a whitespace-significance rule (spaces around comparison) would make whitespace semantically significant across *every* expression to buy a feature inference + type context already cover. Declaration and type positions are already unambiguous, so the whitespace rule stays available as a future escape hatch if explicit call-site args ever prove necessary. — Decided (2026-07-25).
- **Invariant generics** over variance inference — sound by default, no read/write-position analysis (§1).
- **`shared` as an orthogonal capability prefix**, not an `&`-composed marker interface — a param that is both `Comparable` and shareable reads `<shared T: Comparable>` (revises the earlier marker-interface idea in `interfaces.md` §4.4).
