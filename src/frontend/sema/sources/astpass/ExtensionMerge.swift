    ...
    mutating func run() -> Program {
        ...
        for decl in program.decls {
            ...
            case .structDecl(let s):
                ...
                out.append(.structDecl(StructDecl(name: s.name, generics: s.generics, fields: s.fields, properties: mergedProperties(s.properties, into: s.name), methods: methods, conformances: s.conformances + extConformances(s.name), span: s.span)))
            case .enumDecl(let e):
                let methods = merged(e.methods, into: e.name)
                out.append(.enumDecl(EnumDecl(name: e.name, generics: e.generics, cases: e.cases, properties: mergedProperties(e.properties, into: e.name), methods: methods, conformances: e.conformances + extConformances(e.name), span: e.span)))
            case .classDecl(let c):
                let methods = merged(c.methods, into: c.name)
                out.append(.classDecl(ClassDecl(name: c.name, generics: c.generics, fields: c.fields, properties: mergedProperties(c.properties, into: c.name), methods: methods, conformances: c.conformances + extConformances(c.name), span: c.span)))
